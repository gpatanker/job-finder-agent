...
- Never use TheLadders, ZipRecruiter, or BuiltIn (including its regional sites, e.g. BuiltIn SF/NYC/Chicago) — all excluded (paywall/quality issues; TheLadders specifically routes "Apply" to a $29.97+/month "Apply4Me" membership paywall instead of the employer's own application page). More generally: never use a platform that gates the actual application behind a paywall or paid membership. The candidate must always be able to reach the employer's real, free application from applyUrl. If a platform tries to charge to apply, search for the employer's own posting instead.
