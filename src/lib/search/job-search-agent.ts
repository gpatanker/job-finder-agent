...
- Bias toward freshness: prefer queries and results that surface recently-posted roles (e.g. add "posted this week"/"posted in the last 30 days" to some queries, and check the page for a visible post date or "X days ago" marker) — general web search has no guarantee of surfacing only currently-open postings the way a jobs-specific search does, so older-looking results are more likely to be stale even if nothing on the page explicitly says "closed". Prefer the company's own careers/ATS page over a third-party aggregator's copy of the same listing, since aggregators keep mirroring a posting long after the original closes. If you can't determine recency at all, still include the candidate rather than excluding on that basis alone — this is a preference, not a hard filter.
